...
async function testCompleteFlow() {
    try {
        ...
        // Step 7: Verify player was added to participants
        console.log('\n👥 Step 7: Checking tournament participants...');
        const participantsResponse = await axios.get(`http://localhost:3001/api/tournaments/${tournament.id}/participants`, {
            headers: { 'Authorization': `Bearer ${adminToken}` }
        });
        
        const participants = participantsResponse.data;
        console.log(`Tournament now has ${participants.length} participants:`);
        participants.forEach((p, idx) => {
            console.log(`  ${idx + 1}. ${p.player_name} (Rating: ${p.player_rating})`);
        });
